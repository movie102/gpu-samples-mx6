...
/* Construct a surface based on an existing video memory node. */
gceSTATUS
gcoSURF_ConstructWrapperFromVidMemNode(
    IN gcoHAL Hal,
    IN gcuVIDMEM_NODE_PTR Node,
    IN gcePOOL Pool,
    IN gctUINT Bytes,
...
/* Destroy a surface wrapping a video memory node not owned by this process. */
gceSTATUS
gcoSURF_DestroyVidMemNodeWrapper(
    IN gcoSURF Surface
    );
#endif
...
#if gcdDUMP_COMMAND
#   define gcmkDUMPCOMMAND(Os, Buffer, Size, Type) \
        gcmkDUMPBUFFER(Os, Buffer, Size, Type)
#else
#   define gcmkDUMPCOMMAND(Os, Buffer, Size, Type)
#endif
